...
// 專門用來切換 EXR 的函式
function loadEXR(fileName) {
  const loader = new EXRLoader().setPath("./hdr/");
...
// 右下角下拉式選單事件
// ==========================
const exrSelect = document.getElementById("exrSelect");
if (exrSelect) {
  exrSelect.addEventListener("change", () => {
    const file = exrSelect.value;
    loadEXR(file);
  });
}
...
document.getElementById("menu-toggle")?.addEventListener("click", () => {
  const panel = document.getElementById("ui-bar");
